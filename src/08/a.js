...
const isTreeVisible = (row, position) => {
    ...
    const checkTree = (tree) => tree < treeToCheck;
    return (left.every(checkTree) ||
        right.every(checkTree) ||
        top.every(checkTree) ||
        bottom.every(checkTree));
};
let numVisibleTrees = 0;
treeGrid.forEach((row, rowNum, rowArr) => row.forEach((pos, posNum, posArr) => {
    if (rowNum > 0 &&
...
// 1798
//* PART TWO
const getNumVisibleTrees = (trees, treeToCheck) => {
    let count = 0;
...
const getVisibilityScore = (row, position) => {
    const treeToCheck = treeGrid[row][position];
    const verticalRow = treeGrid.map((row) => row[position]);
