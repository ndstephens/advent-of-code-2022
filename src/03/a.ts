...
const getPriorityValue = (item: string) => priorityValue.indexOf(item)! + 1;
...
const priorityItemsSum = input
  ...
  .map((pack) => {
    ...
    const sharedItem = compartment1.filter((item) =>
...
//* PART TWO
let trackerObj = {} as { [key: string]: any };
const sumArr: number[] = [];
...
input.split(/\n/).forEach((pack, i, arr) => {
  ...
  if ((i + 1) % 3 === 0) {
    const commonItem = Object.keys(tempObj).filter(
      (key) => tempObj[key] === 3
    )[0];
