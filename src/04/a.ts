...
const isSectionFullyContained = (arr1: number[], arr2: number[]) => {
  if (isFullyContained(arr1[0], arr2) && isFullyContained(arr1[1], arr2)) {
    return true;
  } else if (
    isFullyContained(arr2[0], arr1) &&
...
const pairedAssignmentsFull = input
  .split(/\n/)
  .map((pair) =>
    pair.split(',').map((sectionRange) => sectionRange.split('-').map(Number))
  )
  .forEach((assignments) => {
...
//* PART TWO
const isPartiallyContained = (val: number, range: number[]) => {
  return val >= range[0] && val <= range[1];
};

const isSectionPartiallyContained = (arr1: number[], arr2: number[]) => {
  if (
    isPartiallyContained(arr1[0], arr2) ||
    isPartiallyContained(arr1[1], arr2)
  ) {
    return true;
  } else if (
    isPartiallyContained(arr2[0], arr1) ||
